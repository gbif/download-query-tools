...
public class HiveSqlQueryTest {
  ...
  public void testSqlTaxonLookup() throws Exception {
    ...
    assertEquals("SELECT scientificname\n" +
            ...
            "WHERE EXISTS(classifications['my-checklist-key'], x -> x IN ('my-taxon-id'))", q.getSql());
  }
  ...
  public void testSqlTaxonLookup2() throws Exception {
    ...
    assertEquals("SELECT scientificname\n" +
            "FROM cattest.occurrence\n" +
            "WHERE EXISTS(classifications['my-checklist-key'], x -> x IN ('my-taxon-id', 'my-taxon-id2'))", q.getSql());
  }
}
